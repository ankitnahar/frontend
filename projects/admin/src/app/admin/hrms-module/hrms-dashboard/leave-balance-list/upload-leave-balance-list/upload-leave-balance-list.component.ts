...
@Component({
  ...
  templateUrl: './upload-leave-balance-list.component.html'
})
export class UploadLeaveBalanceListComponent implements OnInit {
  files: any[] = [];
  ...
  constructor(public dialogRef: MatDialogRef<UploadLeaveBalanceListComponent>, @Inject(MAT_DIALOG_DATA) public data: any,
      ...
      Object.keys(files).forEach(itemUploadFile => {
        ...
        if (ext.toLowerCase() === "csv") {
          if (files.size > AppConstant.TWINTY_FIVE_MP_FILE_ALLOWED) {
            ...
            if (this.uploadDoc) {
              this._commonCrudService.uploadDocument(AdminAPI.LEAVE_BALANCE_UPDATE, this.uploadDoc).subscribe(response => {
                this.onClose(true);
              });
            }
          }
