...
import {BankAccount} from '../../client-module/view-client/update-client/information/bank-information/bank-account.model';
...
@Component({
  ...
  templateUrl: './admin-more-account-type.component.html',
  styleUrls: ['./admin-more-account-type.component.scss']
})
export class AdminMoreAccountTypeComponent extends BaseComponent implements OnInit {
  ...
  getAccountTypeList(pageNumber: number, key?: string, val?: string) {
    this._commonCrudService.listData(AdminAPI.BANK_TYPE_LIST, this.getQueryParams(pageNumber, key, val), this.getSearchParams()).subscribe(Response => {
  ...
  onAccountTypeNameUpdate(type_name, accountTypeObject: any) {
    const params = {'type_name': type_name};
    this._commonCrudService.updateDataWithPut(AdminAPI.BANK_TYPE_LIST, accountTypeObject.id, params).subscribe(Response => {
      this.accountType.map(item => {
        if (item.id === accountTypeObject.id) {
