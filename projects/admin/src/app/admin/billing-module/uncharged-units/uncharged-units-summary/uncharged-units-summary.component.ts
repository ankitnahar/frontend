...
@Component({
  ...
  templateUrl: './uncharged-units-summary.component.html',
  styleUrls: ['./uncharged-units-summary.component.scss']
})
export class UnchargedUnitsSummaryComponent implements OnInit {
  ...
  tabIDInvoice = ADMINTABACCESS.BILLING_INVOICE;
  tabDataInvoice: Privilege | any[];
  ...
  onNewInvoice() {
    const jsonData = convertURLParamToEncode({'service_id': this.service_id, 'entity_id': this.entity_id});
    if (jsonData) {
      this._router.navigate(['/' + AdminRoutes.ADD_NEW_INVOICE], {queryParams: jsonData});
    }
  }

  onUnchargeUnit() {
    this._router.navigate(['/' + AdminRoutes.UNCHARGED_UNITS]);
  }
    ...
    this.route.queryParams
      .subscribe(params => {
        ...
        if (dataItem) {
          ...
          this.PeriodFromValue = (dataItem['from_date']) ? moment(dataItem['from_date']).format('YYYY-MM-DD') : null;
          this.PeriodToValue = (dataItem['to_date']) ? moment(dataItem['to_date']).format('YYYY-MM-DD') : null;
