...
import {payroll_inc_in_ff} from '../../../../../../../utility/constants/base-constants';
...
@Component({
  ...
  templateUrl: './payroll-additional-activity-calculator.component.html',
  providers: [DecimalPipe, NoCommaPipe]
})
export class PayrollAdditionalActivityCalculatorComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._commonCrudService.getData(AdminAPI.BILLING_PAYROLL, this.billingInformation.entity_id, {}).subscribe((response) => {
      ...
      if (this.payrollData.inc_in_ff === 0) {
        if (this.payrollInCInFF.length >= 3) {
          this.payrollInCInFF.splice(1, 1);
        }
      }
    });
    // console.log(this.serviceInfo);
  ...
  initializationMethod() {
    this._commonCrudService.getData(AdminAPI.BILLING_SUBACTIVITY, this.billingInformation.entity_id, {'service_id': 2}).subscribe((response) => {
      ...
      if (itemDataValues) {
        ...
        itemDataValues.forEach(key => {
          ...
          this.bookkeepingSubActivityValues[i]['data'].forEach(keyData => {
            ...
            this.hideShowFields(i, j, keyData['subactivity_code'], keyData['inc_in_ff'], keyData);
  ...
  hideShowFields(parentActivity: number, childActivity: number, categoryCode: number, value: any, keyData?: any) {
      ...
      DataFields.forEach(key => {
        ...
        if (keyData) {
          if (keyData['inc_in_ff'] === 1 && key['is_ff_count'] === 1) {
            this.ffAmountOfSubactivity += Number(keyData[key['key']]);
          }
        }
      });
   ...
   */
  getCountForSubactivityFF() {
    this.ffAmountOfSubactivity = 0;
    const dataItem = this.getFilterFieldArray().controls;
    let i = 0;
    if (dataItem) {
      dataItem.forEach(itemData => {
        const subItem = this.getFilterFieldArrayData(i).controls;
        if (subItem) {
          let j = 0;
          subItem.forEach(subItemData => {
            const subItemField = this.getFilterFieldArrayDataFields(i, j).controls;
            if (subItemField) {
              subItemField.forEach(dataValue => {
                if (subItem) {
                  if (subItemData.get('inc_in_ff').value === 1 && dataValue.get('is_ff_count').value === 1) {
                    this.ffAmountOfSubactivity += Number(subItemData.get(dataValue.get('key').value).value);
                  }
                }
              });
            }
  ...
  initDisplayField(item?: any) {
    return this._fb.group({
      ...
      is_ff_count: new FormControl(item ? item['is_ff_count'] : 0),
    });
  }
  ...
  getYesNoStatus(status_id: number): string {
    const val = this.payrollInCInFF.filter(elem => elem.key === Number(status_id));
