...
@Component({
  selector: 'app-edit-task-checklist-peer-review',
  templateUrl: './edit-task-checklist-peer-review.component.html',
  styleUrls: ['./edit-task-checklist-peer-review.component.scss'],
  providers: [CommonCrudService]
})

export class EditTaskChecklistPeerReviewComponent extends BaseComponent implements OnInit {
// State variables
  ...
  ngOnInit() {
    this.worksheetData = this._sharedService.getPeerReviewChecklistData();
  ...
  initSection(item ?: any) {
    return this._fb.group({
      ...
      is_checked: new FormControl(item ? item['is_checked'] : null),
  ...
  initQuestions(item?: any) {
    ...
    return this._fb.group({
      ...
      reviewer_comment: new FormControl(item ? item['reviewer_comment'] : null),
      review_tag_id: new FormControl(item ? item['review_tag_id'] : null),
      training_id: new FormControl(item ? this.getArrayToString(item['training_id'], ',') : null),
      ...
      technical_head_comment: new FormControl(item ? item['technical_head_comment'] : null, (item['technical_head_action'] === 2 || item['technical_head_action'] === 3) ? [<any> Validators.required, <any>Validators.pattern(CommonRegex.NOT_ALLOWED_FIRST_AND_LAST_WHITESPACE_REGEXP)] : null),
    });
  }
  ...
  onUploadDocument(event, id) {
    const workSheetData = this._sharedService.getPeerReviewChecklistData().id;
  ...
  updateCheckGroupQuestionValidation(parent: number, is_checked: number) {
    ...
    if (GroupQuestions) {
      ...
      GroupQuestions.forEach(item => {
        if (is_checked === 0) {
          item.get('technical_head_action').setValidators(null);
          item.get('technical_head_action').updateValueAndValidity();
        } else if (is_checked === 1) {
          item.get('technical_head_action').setValidators(<any> Validators.required);
          item.get('technical_head_action').updateValueAndValidity();
        }
        child++;
      });
