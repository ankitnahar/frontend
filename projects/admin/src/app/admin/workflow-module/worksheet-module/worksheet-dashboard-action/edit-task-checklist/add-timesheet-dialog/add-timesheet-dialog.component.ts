...
export class AddTimesheetDialogComponent extends BaseComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<AddTimesheetDialogComponent>,
  ...
  createTimesheetForm() {
    this.addTimesheetForm = this._fb.group({
      ...
      is_explode_worksheet: new FormControl(0),
    });
   ...
   * @param value
   */
  getSubactivityFilterList(value: any, is_explode?: number) {
    // Get Subactivity List
    if (value) {
      const isArray = (is_explode === 1) ? value.split('::') : value;
      const id = (is_explode === 1) ? isArray[0] : value;
      this._sharedObjService.getSubactivity({
        'records': 'all',
        'sortBy': 'asc',
        'sortOrder': 'subactivity_code'
      }, {'compare': {'equal': {'task_id': id}, 'notequal': {'subactivity_code': 402}}}).subscribe((response) => {
      ...
      this._sharedObjService.getSubactivity({
        ...
        'sortBy': 'asc',
        'sortOrder': 'subactivity_code'
      }, {'compare': {'notequal': {'subactivity_code': 402}}}).subscribe((response) => {
        this.subActivityList = response;
      });
    }
  }
   ...
   * Change Task Type
   */
  changeTaskType(value: boolean) {
    if (value) {
      this.isChangeTask = 1;
      if (this.isChangeTask === 1) {
        // FETCH_OTHER_WORKSHEET
        this._commonCrudService.listData(AdminAPI.FETCH_OTHER_WORKSHEET + '/' + this.worksheetData.entity_id, {
  ...
  onSubmitTimesheet(form: FormGroup) {
    if (form.valid) {
      ...
      form.value['is_export'] = this.isChangeTask;
