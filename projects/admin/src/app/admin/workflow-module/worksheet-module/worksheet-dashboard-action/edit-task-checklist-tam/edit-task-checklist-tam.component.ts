...
import {EmailPreview, WorksheetDocument, WorksheetListing, WorksheetNotes} from '../../worksheet-dashboard-tab/worksheet.model';
...
@Component({
  selector: 'app-edit-task-checklist-tam',
  templateUrl: './edit-task-checklist-tam.component.html',
  styleUrls: ['./edit-task-checklist-tam.component.scss']
})
export class EditTaskChecklistTamComponent extends BaseComponent implements OnInit {
// State variables
  ...
  ngOnInit() {
    this.worksheetData = this._sharedService.getTamReviewChecklistData();
  ...
  submitChecklist(form: FormGroup) {
    if (form.valid && this.worksheetData.status_id.id === 15) {
      this._sharedService.setReviewChecklistData(null);
      this._sharedService.setReviewChecklistData(this.worksheetData);
      this._router.navigate([AdminRoutes.TASK_CHECKLIST_EMAIL_REVIEW]);
    } else {
      this._sharedService.setChecklistData(null);
      this._sharedService.setChecklistData(this.worksheetData);
  ...
  initQuestions(item?: any) {
    ...
    return this._fb.group({
      ...
      team_member_action: new FormControl(item ? item['team_member_action'] : null),
      ...
      technical_head_comment: new FormControl(item ? item['technical_head_comment'] : null),
    });
  }
  ...
  onUploadDocument(event, id) {
    const workSheetData = this._sharedService.getTamReviewChecklistData().id;
