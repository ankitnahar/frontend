...
@Component({
  ...
  templateUrl: './add-new-reviewe-timesheet-dialog.component.html'
})
export class AddNewRevieweTimesheetDialogComponent extends BaseComponent implements OnInit {
  ...
  nameOfEmp = [];
  userData: AdminUser;
  reviewCodeData = [405, 417, 422, 462, 463, 464, 468, 711];
  hrDetail: AttendanceSummary;

  constructor(public dialogRef: MatDialogRef<AddNewRevieweTimesheetDialogComponent>,
  ...
  createTimesheetForm() {
    this.addTimesheetForm = this._fb.group({
      ...
      subactivity_code: new FormControl((this.timeSheetData) ? ((this.timeSheetData.subactivity_code === 404) ? 402 : (this.timeSheetData.subactivity_code === 701) ? 711 : 0 ) : 0, <any>Validators.required),
  ...
  hideShowFields(value: any) {
    ...
    if (value) {
      const dataOfReview = this.reviewCodeData.indexOf(this.timeSheetData.subactivity_code);
