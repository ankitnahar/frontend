...
import {Views} from '../../worksheet-dashboard-tab/myworksheet/myworksheet.component';
import {ReviewActionAllocateReviewerDialog} from './review-action-allocate-reviewer-dialog/review-action-allocate-reviewer-dialog';
...
import {CommonFunctions, convertURLParamToDecode, convertURLParamToEncode} from '../../../../../../utility/common-functions';
...
@Component({
  ...
  templateUrl: './review-or-knock-back-worksheet.component.html',
  styleUrls: ['./review-or-knock-back-worksheet.component.scss'],
})
export class ReviewOrKnockBackWorksheetComponent extends BaseComponent implements OnInit {
  taskData: any;
  // Constant Variables
  enumView = Views;
  activeView: Views;
  ...
  tabID = ADMINTABACCESS.WORKFLOW_REVIEWANDKNOCKBACKWORKSHEET;
  tabData: Privilege | any[];
  worksheetTabID = ADMINTABACCESS.WORKFLOW_WORKSHEET;
  todayDate = '';
  typeData = '';
  userInfo: AdminUser;
  selectedFilterEntity = 0;
  selectedFilterFrequency = 0;
  ...
  get entity_grouptype_id(): AbstractControl {
  ...
  ngOnInit() {
    ...
    this.route.queryParams
      .subscribe(params => {
        ...
        if (dataItem) {
          this.selectedFilterEntity = (dataItem['entity_id']) ? dataItem['entity_id'] : null;
        }
      });
  ...
  getWorksheetListing(pageNumber: number, key?: string, val?: string) {
    this._commonCrudService.listData(AdminAPI.REVIEW_OR_KNOCKBACK_WORKSHEET_LISTING, this.getQueryParams(pageNumber, key, val), this.getSearchParam()).subscribe(response => {
      this.handleWorksheetRespone(response);
    });
    // Get Counter
    this._commonCrudService.listData(AdminAPI.REVIEW_OR_KNOCKBACK_WORKSHEET_LISTING, {
      'type': 'allocate',
      'action': 'count'
    }, {}).subscribe(response => {
  ...
  createAdvanceFilterForm() {
    this.filterForm = this._fb.group({
      ...
      frequency_id: new FormControl((this.selectedFilterFrequency > 0) ? this.selectedFilterFrequency : null),
      ...
      entity_grouptype_id: new FormControl(null),
    });
  ...
  downloadExcel() {
    ...
    this._commonCrudService.downloadExcelData(AdminAPI.REVIEW_OR_KNOCKBACK_WORKSHEET_LISTING, params, this.getSearchParam(), 'Review Or Knoackback Worksheet ', 0).subscribe(response => {
    });
  }
   ...
   * @param value
   */
  onChangeUpdateFilterField(value: number) {
    if (value === 0) {
      this.typeData = null;
    } else if (value === 1) {
   ...
   */
  onEditTaskCheckList(data: WorksheetListing) {
    if (data) {
  ...
  getClientBelongsToName(id: number): string {
    const val = this.entityGroupType.filter(elem => elem.id === Number(id));
