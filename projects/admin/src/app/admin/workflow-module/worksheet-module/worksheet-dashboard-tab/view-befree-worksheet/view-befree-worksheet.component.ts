...
@Component({
  ...
  templateUrl: './view-befree-worksheet.component.html',
  styleUrls: ['./view-befree-worksheet.component.scss']
})
export class ViewBefreeWorksheetComponent extends BaseComponent implements OnInit {
  @Input() befreeWorksheetCount: any;
  taskData: any;
  // Constant Variables
  enumView = Views;
  activeView: Views;
  ...
  tabID = ADMINTABACCESS.WORKFLOW_VIEWBEFREEWORKSHEET;
  ...
  getWorksheetListing(pageNumber: number, key?: string, val?: string) {
    this._commonCrudService.listData(AdminAPI.BEFREE_WORKSHEET_LISTING, this.getQueryParams(pageNumber, key, val), this.getSearchParam()).subscribe(response => {
   ...
   */
  downloadExcel() {
    const params = {'excel': 1, 'records': 'all', 'type': 'befree'};
    if (this.tamId) {
      params['technical_account_manager'] = this.tamId;
    }
    ...
    this._commonCrudService.downloadExcelData(AdminAPI.BEFREE_WORKSHEET_LISTING, params, this.getSearchParam(), 'Befree Worksheet ', 0).subscribe(response => {
    });
  }
