...
@Component({
  ...
  templateUrl: './view-incompleted.component.html',
  styleUrls: ['./view-incompleted.component.scss']
})
export class ViewIncompletedComponent extends BaseComponent implements OnInit {
  @Input() incompleteWorksheetCount: any;
  @Input() isTab: any;
  taskData: any;
  // Constant Variables
  enumView = Views;
  ...
  tabID = ADMINTABACCESS.WORKFLOW_VIEWINCOMPLETEDWORKSHEET;
  ...
  get critical_task(): AbstractControl {
  ...
  ngOnInit() {
    ...
    this.route.queryParams
      .subscribe(params => {
        ...
        if (dataItem) {
          this.selectedEntityFilterData = (dataItem['entity_id']) ? Number(dataItem['entity_id']) : null;
        }
      });
  ...
  getDropdownData() {
    ...
    this._sharedObjService.getUserList({'records': 'all',"field":"user.id,user.userfullname"}, {compare:{equal:{is_active : 1}}}).subscribe((response) => {
      ...
      this.tlList = this.userList.filter(data => (data['designation_id']) ? +data['designation_id']=== 60 : 0);
      this.atlList = this.userList.filter(data => (data['designation_id']) ? +data['designation_id']=== 61 : 0);
    });
  ...
  createAdvanceFilterForm() {
    this.filterForm = this._fb.group({
      ...
      entity_id: new FormControl((this.selectedEntityFilterData > 0) ? this.selectedEntityFilterData : null),
      master_activity_id: new FormControl(null),
      task_id: new FormControl(null),
      frequency_id: new FormControl(null),
      start_date: new FormControl(null),
      end_date: new FormControl(null),
      due_date_from: new FormControl(this.previousDate),
      due_date_to: new FormControl(this.nextDate),
      ...
      critical_task: new FormControl(null)
    });
  ...
  downloadExcel() {
    ...
    this._commonCrudService.downloadExcelData(AdminAPI.INCOMPLETE_WORKSHEET_LISTING, params, this.getSearchParam(), 'Incompleted Worksheet ', 0).subscribe(response => {
    });
  }
  ...
  onCompleteWorksheetDiloag() {
    let dialogRef = this.dialog.open(CompleteWorksheetStatusDialogComponent, {
