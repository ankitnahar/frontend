...
import {BASE, CLFIELDTYPE, DDFIELDTYPE, FIELDTYPE, TBFIELDTYPE, ticketPriority, ticketSeverity, ticketTopic, ticketTypeOfMistake, TNFIELDTYPE, ToastType, yesNo} from '../../../../utility/constants/base-constants';
...
export class TicketReportComponent implements OnInit, OnDestroy {
  ...
  tabApiURL = '';
  tabReportName = '';
  BIR = ADMINTABACCESS.REPORT_TICKETREPORT;
  ...
  getTeam() {
    this._commonCrudService.listData(AdminAPI.TEAM, {}, {}).subscribe(Response => {
      Response.payload.data.map(item => {
        this.teamList.push({key: item.id, label: item.name});
      });
    });
  }
  ...
  getTicketStatusList() {
    this._commonCrudService.listData(AdminAPI.DROPDOWN_LIST, {
      ...
      if (Response) {
        Response.map(item => {
          this.ticketStatusList.push({key: item.state_id, label: item.status});
        });
      }
    });
  }
  ...
  getTicketTypeList() {
    this._commonCrudService.listData(AdminAPI.TICKET_TYPE, {
      ...
      Response.payload.data.map(item => {
        this.ticketTypeList.push({key: item.id, label: item.name});
      });
    });
  }
  ...
  getTabReportAPIURL(tabID) {
    if (+tabID === this.BIR) {
      this.tabApiURL = AdminAPI.TICKET_GENERATE_REPORT;
  ...
  getClientFilterFiled(clientTypeId: number) {
    this._commonCrudService.listData(AdminAPI.CLIENT_REPORT_FILTER_FIELD + '/' + clientTypeId, {'type': 'filter'}).subscribe(Response => {
      ...
      this.rpageIndex = this.rpage - 1;
      this.rtotalRecords = +Response.pager.totalRecords;
      if (+this.reportTabData['id'] === this.BIR) {
        this.ReportList.map(item => {
          if (item.field_value) {
            ...
            if (item.field_value === 'yesNo') {
              ...
              item.field_value_array = ticketSeverity.slice(1);
            } else if (item.field_value === 'ticketpriority') {
              item.field_value_array = ticketPriority.slice(1);
            } else if (item.field_value === 'tickettypeofmistake') {
              item.field_value_array = ticketTypeOfMistake.slice(1);
            } else if (item.field_value === 'tickettopic') {
              item.field_value_array = ticketTopic.slice(1);
            } else {
            ...
            if (item.field_name === 'code' || item.field_name === 'entity_name'
              ...
              item.isCommaSeperated = false;
              item.field_value_array = this.ticketTypeList;
            } else if (item.field_name === 'team_id') {
              item.isMultiSelect = false;
              item.isCommaSeperated = false;
              item.field_value_array = this.teamList;
            } else if (item.field_name === 'status_id') {
              item.isMultiSelect = false;
              item.isCommaSeperated = false;
              item.field_value_array = this.ticketStatusList;
            } else if (item.field_name === 'technical_account_manager') {
              item.isMultiSelect = false;
              item.isCommaSeperated = false;
              const itemArray = this.userList.filter(itemData => (itemData['designation_id']) ? itemData['designation_id']['id'] === Number(9) : 0);
              if (itemArray.length) {
                this.tamList = [];
                itemArray.map(itemArrayData => {
                  this.tamList.push({'key': itemArrayData['id'], 'label': itemArrayData['userfullname']});
                });
              } else {
                this.tamList = [];
              }
              item.field_value_array = this.tamList;
            } else if (item.field_name === 'technical_head') {
              item.isMultiSelect = false;
              item.isCommaSeperated = false;
              const itemArray = this.userList.filter(itemData => (itemData['designation_id']) ? itemData['designation_id']['id'] === Number(63) : 0);
              if (itemArray.length) {
                this.thList = [];
                itemArray.map(itemArrayData => {
                  this.thList.push({'key': itemArrayData['id'], 'label': itemArrayData['userfullname']});
                });
              } else {
                this.thList = [];
              }
              item.field_value_array = this.thList;
            } else if (item.field_name === 'staff_incharge' || item.field_name === 'staff_involved_issue' || item.field_name === 'ticket_assignee') {
              item.isMultiSelect = true;
              item.isCommaSeperated = false;
              item.field_value_array = this.userList;
            }
          }
