...
import {BASE, CLFIELDTYPE, DDFIELDTYPE, FIELDTYPE, TBFIELDTYPE, TNFIELDTYPE, ToastType, yesNo, yesNoNa, yesNoOther} from '../../../../utility/constants/base-constants';
...
@Component({
  ...
  templateUrl: './billing-service-report.component.html',
  styleUrls: ['./billing-service-report.component.scss']
})
export class BillingServiceReportComponent implements OnInit, OnDestroy {
  ...
  tabApiURL = '';
  tabReportName = '';
  BSR = ADMINTABACCESS.REPORT_BILLINGSERVICEREPORT;
  ...
  getPayrollCalc() {
    this._commonCrudService.listData(AdminAPI.DROPDOWN_LIST, {
      ...
      if (Response) {
        Response.map(item => {
          this.payrollCalcList.push({key: item.id, label: item.name});
        });
      }
    });
  }
  ...
  getStates() {
    this._commonCrudService.listData(AdminAPI.DROPDOWN_LIST, {
      ...
      if (Response) {
        Response.map(item => {
          this.stateList.push({key: item.state_id, label: item.state_name});
        });
      }
    });
  }
  ...
  getPlan() {
    this._commonCrudService.listData(AdminAPI.BILLING_SUBSCRIPTION_PLAN, {'records': 'all'}, {}).subscribe((response) => {
      if (response.payload.data) {
        response.payload.data.map(item => {
          this.planListAll.push({key: item.id, label: item.software_plan});
        });
      }
    });
  }
  ...
  getSoftware() {
    this._commonCrudService.listData(AdminAPI.BILLING_SUBSCRIPTION_SOFTWARE, {'records': 'all'}, {}).subscribe((response) => {
      if (response.payload.data) {
        response.payload.data.map(item => {
          this.softwareList.push({key: item.id, label: item.software_plan});
        });
      }
    });
  }
  ...
  getRecurring() {
    this._commonCrudService.listData(AdminAPI.INVOICE_RECURRING_LIST, {'records': 'all'}, {}).subscribe((response) => {
      if (response.payload.data) {
        response.payload.data.map(item => {
          this.recurringList.push({key: item.id, label: item.recurring_name});
        });
      }
    });
  }
  ...
  getTabReportAPIURL(tabID) {
    if (+tabID === this.BSR) {
      this.tabApiURL = AdminAPI.BILLING_SERVICE_GENERATE_REPORT;
  ...
  getClientFilterFiled(clientTypeId: number) {
    this._commonCrudService.listData(AdminAPI.CLIENT_REPORT_FILTER_FIELD + '/' + clientTypeId, {'type': 'filter'}).subscribe(Response => {
      ...
      this.rpageIndex = this.rpage - 1;
      this.rtotalRecords = +Response.pager.totalRecords;
      if (+this.reportTabData['id'] === this.BSR) {
        this.ReportList.map(item => {
          if (item.field_value) {
            ...
            if (item.field_name === 'code' || item.field_name === 'entity_name'
              ...
              item.isMultiSelect = false;
              item.isCommaSeperated = false;
              item.field_value_array = this.recurringList;
              ...
              item.isMultiSelect = false;
              item.isCommaSeperated = false;
              item.field_value_array = this.softwareList;
            } else if (item.field_name === 'plan_id') {
              item.isMultiSelect = false;
              item.isCommaSeperated = false;
              item.field_value_array = this.planListAll;
            } else if (item.field_name === 'state_id') {
              item.isMultiSelect = false;
              item.isCommaSeperated = false;
              item.field_value_array = this.stateList;
            } else if (item.field_name === 'calc_id') {
              item.isMultiSelect = false;
              item.isCommaSeperated = false;
              item.field_value_array = this.payrollCalcList;
            }
