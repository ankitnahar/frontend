...
@Component({
  ...
  templateUrl: './billing-subactivity-report.component.html',
  styleUrls: ['./billing-subactivity-report.component.scss']
})
export class BillingSubactivityReportComponent implements OnInit, OnDestroy {
  ...
  tabApiURL = '';
  tabReportName = '';
  BSAR = ADMINTABACCESS.REPORT_BILLINGSUBACTIVITYREPORT;
  ...
  getSubactivity() {
    this._commonCrudService.listData(AdminAPI.DROPDOWN_LIST, {
      ...
      if (response) {
        response.map(item => {
          this.subactivityList.push({key: item.subactivity_code, label: item.subactivity_full_name});
        });
      }
    });
  }
  ...
  getTabReportAPIURL(tabID) {
    if (+tabID === this.BSAR) {
      this.tabApiURL = AdminAPI.BILLING_SUBACTIVITY_GENERATE_REPORT;
  ...
  getClientFilterFiled(clientTypeId: number) {
    this._commonCrudService.listData(AdminAPI.CLIENT_REPORT_FILTER_FIELD + '/' + clientTypeId, {'type': 'filter'}).subscribe(Response => {
      ...
      this.rpageIndex = this.rpage - 1;
      this.rtotalRecords = +Response.pager.totalRecords;
      if (+this.reportTabData['id'] === this.BSAR) {
        this.ReportList.map(item => {
          if (item.field_value) {
            ...
            if (item.field_name === 'code' || item.field_name === 'entity_name'
              ...
              item.isMultiSelect = false;
              item.isCommaSeperated = false;
              item.field_value_array = this.subactivityList;
            }
