...
import {BASE, category, CLFIELDTYPE, DDFIELDTYPE, FIELDTYPE, fulltimeresource, payment, TBFIELDTYPE, TNFIELDTYPE, ToastType, yesNo, yesNoNa, yesNoOther} from '../../../../utility/constants/base-constants';
...
export class BillingReportComponent implements OnInit, OnDestroy {
  ...
  tabApiURL = '';
  tabReportName = '';
  BIR = ADMINTABACCESS.REPORT_BILLINGREPORT;
  ...
  getPlan() {
    this._commonCrudService.listData(AdminAPI.BILLING_SUBSCRIPTION_PLAN, {}, {}).subscribe((response) => {
      if (response.payload.data) {
        response.payload.data.map(item => {
          this.planListAll.push({key: item.state_id, label: item.state_name});
        });
      }
    });
  }
  ...
  getSoftware() {
    this._commonCrudService.listData(AdminAPI.BILLING_SUBSCRIPTION_SOFTWARE, {}, {}).subscribe((response) => {
      if (response.payload.data) {
        response.payload.data.map(item => {
          this.softwareList.push({key: item.state_id, label: item.state_name});
        });
      }
    });
  }
  ...
  getBillingCard() {
    this._commonCrudService.listData(AdminAPI.DROPDOWN_LIST, {
      ...
      if (Response) {
        Response.map(item => {
          this.cardNumberList.push({key: item.id, label: item.name});
        });
      }
    });
  }
  ...
  getTabReportAPIURL(tabID) {
    if (+tabID === this.BIR) {
      this.tabApiURL = AdminAPI.BILLING_GENERATE_REPORT;
        ...
        this.ReportList.map(item => {
          if (item.field_value) {
            ...
            if (item.field_value === 'yesNo') {
              ...
              item.field_value_array = category.slice(1);
            } else if (item.field_value === 'fulltimeresource') {
              item.field_value_array = fulltimeresource.slice(1);
            } else if (item.field_value === 'payment') {
              item.field_value_array = payment.slice(1);
            } else {
            ...
            if (item.field_name === 'code' || item.field_name === 'entity_name'
              ...
              item.isMultiSelect = false;
              item.isCommaSeperated = false;
              item.field_value_array = this.cardNumberList;
            }
