...
import {BASE, CLFIELDTYPE, DDFIELDTYPE, FIELDTYPE, taxCondition, TBFIELDTYPE, TNFIELDTYPE, ToastType, yesNo, yesNoNa, yesNoOther} from '../../../../utility/constants/base-constants';
...
@Component({
  ...
  templateUrl: './billing-tax-turnover-report.component.html',
  styleUrls: ['./billing-tax-turnover-report.component.scss']
})
export class BillingTaxTurnoverReportComponent implements OnInit, OnDestroy {
  ...
  tabApiURL = '';
  tabReportName = '';
  BTTR = ADMINTABACCESS.REPORT_BILLINGTAXTURNOVER;
  ...
  getTabReportAPIURL(tabID) {
    if (+tabID === this.BTTR) {
      this.tabApiURL = AdminAPI.BILLING_TAX_TURNOVER_GENERATE_REPORT;
  ...
  getClientFilterFiled(clientTypeId: number) {
    this._commonCrudService.listData(AdminAPI.CLIENT_REPORT_FILTER_FIELD + '/' + clientTypeId, {'type': 'filter'}).subscribe(Response => {
      ...
      this.rpageIndex = this.rpage - 1;
      this.rtotalRecords = +Response.pager.totalRecords;
      if (+this.reportTabData['id'] === this.BTTR) {
        this.ReportList.map(item => {
          if (item.field_value) {
            ...
            if (item.field_value === 'yesNo') {
              ...
              item.field_value_array = taxCondition;
            } else {
