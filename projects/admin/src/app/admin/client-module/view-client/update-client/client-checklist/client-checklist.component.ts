...
import {AddClientChecklistQuestionDialog} from './add-question-checklist-dialog/add-question-dialog.component';
...
export class ClientChecklistComponent implements OnInit {
  ...
  tabID = ADMINTABACCESS.CLIENT_CLIENTCHECKLIST;
  tabData: Privilege | any[];
  tabQuestID = ADMINTABACCESS.CLIENT_ENTITYCHECKLISTQUESTION;
  tabQuestData: Privilege | any[];
  ...
  ngOnInit() {
    ...
    this.tabQuestData = this._sharedService.checkUserPrivilegesTabs(this.tabQuestID);
  ...
  /**
   * On adding or updating questions on questionView(this view comes after adding or updating checklist)
   */
  addUpdateQuestionView() {
    const params = {
      entity_checklist_id: this.selectedChecklist.id,
      question: JSON.stringify(this.questionViewObject)
    };
    if (this.availableQuestions === 0) {
      this._commonCrudService.addData(AdminAPI.ADD_CLIENT_QUESTION_VIEW + '/' + this.clientInformation.id, params)
        .subscribe(() => {
          this.getQuestionViewList(this.selectedChecklist['id']);
        });
    } else {
      params['_method'] = 'put';
      this._commonCrudService.updateData(AdminAPI.UPDATE_CLIENT_QUESTION_VIEW, this.clientInformation.id, params)
        .subscribe(() => {
          this.getQuestionViewList(this.selectedChecklist['id']);
        });
    }
  ...
  /**
   *  Event on Adding or updating client checklist
   */
  onAddUpdateChecklist() {
    let params = {
      checklist: JSON.stringify(this.checklistArray)
    };
    if (this.isNewChecklist === 0) {
      this._commonCrudService.addData(AdminAPI.ADD_CLIENT_CHECKLIST_DATA + '/' + this.clientInformation.id, params)
        .subscribe(() => {
          this.handleAddUpdateChecklistResponse();
        });
    } else {
      params['_method'] = 'put';
      this._commonCrudService.updateData(AdminAPI.UPDATE_CLIENT_CHECKLIST_DATA, this.clientInformation.id, params)
        .subscribe(() => {
          this.handleAddUpdateChecklistResponse();
        });
    }
  ...
  /**
   * Event when filter view is opened, on client question list view
   */
  onOpenFilter() {
  ...
  /**
   * Event when filter view is closed, on client question list view
   */
  onCloseQuestionFilter() {
    this.isFilterView = false;
  }

  /**
   * Function gets called when clicked on 'Add Question' on questionView and opens a Dialog
   */
  openDialog() {
    const dialogConfigData: any = {
      panelClass: 'add-checklist-question-panel-container',
      data: {
        checklistList: this.questionViewChecklist
      }
    };
    const dialogRef = this.dialog.open(AddClientChecklistQuestionDialog, dialogConfigData);
    dialogRef.afterClosed().subscribe((checklist: QuestionViewChecklist) => {
      if (checklist) {
        this.selectedChecklist = checklist;
        this.getQuestionViewList(checklist.id);
      }
