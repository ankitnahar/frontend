...
@Component({
  ...
  templateUrl: './view-discontinue-client-details.component.html',
  styleUrls: ['./view-discontinue-client-details.component.scss']
})
export class ViewDiscontinueClientDetailsComponent extends BaseComponent implements OnInit {
  ...
  createAddDiscontinueQuestionForm() {
    if (this.clientDiscontinueStage === 5) {
      this.addDiscontinueQuestionForm = this._fb.group({
        ...
        discontinue_comment_by_qc: new FormControl(this.clientDiscontinue.discontinue_comment_by_qc)
      });
    } else if (this.clientDiscontinueStage === 6) {
      this.addDiscontinueQuestionForm = this._fb.group({
        question_json: this._fb.array([]),
        discontinue_comment_by_sales: new FormControl(this.clientDiscontinue.discontinue_comment_by_sales)
      });
  ...
  createQuestionGroup(item ?: any) {
    return this._fb.group({
      ...
      is_checked: new FormControl(item ? item['is_checked'] : '', <any> Validators.required),
