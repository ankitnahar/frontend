...
import {QueryDataList, QueryStatus} from '../query.model';
...
@Component({
  ...
  templateUrl: './pending-queries-list.component.html',
  styleUrls: ['./pending-queries-list.component.scss']
})
export class PendingQueriesListComponent implements OnInit, OnChanges {
  @Input() querySelectedStatus: any;
  @Input() queryselectedTabID: any;
  equalJSON = {};
  likeJSON = {};
  inJSON = {};
  findInSetJSON = {};
  ...
  // Data Variables
  queryList: QueryDataList[] = [];
  queryStatusList: QueryStatus[] = [];
  trIndex = -1;
  ...
  getQueryStatusList() {
    this._commonCrudService.listData(AdminAPI.QUERY_STAGE_LIST, {}, {}).subscribe(response => {
      this.queryStatusList = response.payload.data;
    });
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes['querySelectedStatus']) {
      if (!isUndefined(this.querySelectedStatus)) {
   ...
   * @param {string} val
   */
  getQueryList(pageNumber: number, key?: string, val?: string) {
    this._commonCrudService.listData(AdminAPI.QUERY_LIST + '/' + this.querySelectedStatus, this.getQueryParams(pageNumber, key, val), this.getSearchParam()).subscribe((response) => {
  ...
  downloadExcel() {
    ...
    this._commonCrudService.downloadExcelData(AdminAPI.QUERY_LIST + '/' + this.querySelectedStatus, params, this.getSearchParam(), 'Query List ', 0).subscribe(response => {
    });
  }
   ...
   */
  onConfirmationDialog(type: number, moveTo: string, queryData: QueryDataList) {
    ...
    dialogRef.afterClosed().subscribe((value) => {
      if (value) {
        if (type === 1) {
          ...
          param['type'] = 1;
          param['stage_id'] = 2;
          this._commonCrudService.updateData(AdminAPI.QUERY_MOVE_TO_TL, queryData.id, param).subscribe((response) => {
            this.getQueryList(1, 'id', 'desc');
          });
        } else if (type === 2) {
          const param = {};
   ...
   * @param queryData
   */
  onDeleteQuery(queryData: QueryDataList) {
    const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      data: {
    ...
    dialogRef.afterClosed().subscribe((value) => {
      if (value) {
        this._commonCrudService.deleteData(AdminAPI.QUERY_LIST, queryData.id).subscribe(Response => {
          this.getQueryList(1, 'id', 'desc');
        });
      }
    });
   ...
   * @param queryData
   */
  OnEditQuery(queryData: QueryDataList) {
    this._sharedService.setClientData(GLOBALDATAKEYS.QUERY_MODULE, null);
    this._sharedService.setClientData(GLOBALDATAKEYS.QUERY_MODULE, queryData);
   ...
   * @param queryData
   */
  onViewQuery(queryData: QueryDataList) {
    this._sharedService.setClientData(GLOBALDATAKEYS.QUERY_MODULE, null);
    this._sharedService.setClientData(GLOBALDATAKEYS.QUERY_MODULE, queryData);
    this._router.navigate([]).then(result => {
      window.open('/' + AdminRoutes.VIEW_QUERIES, '_blank');
    });
  }
   ...
   * @param queryData
   */
  onQueryLog(queryData: QueryDataList) {
    const dialogRef = this.dialog.open(QueryLogDialogComponent, {
  ...
  onQueryReminderLog(queryData: QueryDataList) {
    const dialogRef = this.dialog.open(QueryReminderLogDialogComponent, {
      panelClass: 'add-form-medium-dialog-container',
      data: {
        queryData: queryData
      }
   ...
   * @param Query
   */
  onSnoozeQueryDialog(queryData: QueryDataList) {
    const dialogRef = this.dialog.open(SnoozeQueryDialogComponent, {
