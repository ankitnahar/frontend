...
@Component({
  ...
  templateUrl: './upload-query-documents-dialog.component.html'
})
export class UploadQueryDocumentsDialogComponent implements OnInit {
  ...
  queryDetailId = 0;
  constructor(public dialogRef: MatDialogRef<UploadQueryDocumentsDialogComponent>,
  ...
  createFolderListForMoveFile() {
    const params = {'entity_id': this.queryData.entity_id};
    // console.log(1);
  ...
  googleDriveList(entity_id: number, parent_id: number, subclient_id: number) {
    ...
    this._commonCrudService.listData(AdminAPI.GOOGLE_DRIVE_LIST, params).subscribe((response) => {
      ...
      if (parent_id === 0 && subclient_id === 0) {
        ...
        this.googleDriveAllFileFolder.entity_id = this.queryData.entity_id;
  ...
  createAdvanceFilterForm() {
    this.filterForm = this._fb.group({
      entity_id: new FormControl(this.queryData.entity_id, Validators.required),
