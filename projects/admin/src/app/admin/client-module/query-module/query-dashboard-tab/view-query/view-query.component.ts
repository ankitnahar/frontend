...
import {AdditionalQueryRequired, QueryDataList} from "../query.model";
...
import {SendToClientDialogComponent} from "../../../information-required/information-required-tab/update-information/send-to-client-dialog/send-to-client-dialog.component";
import {SendBackToStaffDialogComponent} from "../../../information-required/information-required-tab/update-information/send-back-to-staff-dialog/send-back-to-staff-dialog.component";
...
export class ViewQueryComponent extends BaseComponent implements OnInit {
  ...
  createEditQueryForm() {
    this.editQueryForm = this._fb.group({
      ...
      reminder: new FormControl(this.queryData.reminder),
  ...
  createQueryDetailItem(item ?: any) {
    ...
    return this._fb.group({
      ...
      is_skip: new FormControl(item.is_skip),
  ...
  getDocumentListRefresh(queryDetailId: number, index: number, child: number) {
    this._commonCrudService.getData(AdminAPI.QUERY_VIEW, this.queryData.id).subscribe((responseData) => {
